...
const MISSOES = {
  plastic: {
    ...
    desc: "Leve pelo menos 2kg de plástico a qualquer EcoPonto. O voucher é creditado automaticamente quando o ponto confirmar o recebimento — sem precisar fazer nada!",
    mat: ["♻️ Plástico"],
  },
  vidro: {
    tag: "🔵 Bônus Triplo",
    titulo: "Levar Vidro ao Ponto",
    desc: "Missão especial de bônus triplo! Leve qualquer quantidade de vidro ao Coleta Norte ou Shopping Coleta. VoucherVerde creditado em até 24h após confirmação do ponto.",
    mat: ["🔵 Vidro"],
  },
  eletro: {
    tag: "🔋 Missão Ativa",
    titulo: "Descarte Eletrônico",
    desc: "Leve pelo menos 3 itens eletrônicos (celulares, carregadores, baterias) ao Ponto Sul. Cada item confirmado gera R$5 de voucher automaticamente no seu app.",
    mat: ["🔋 Eletrônico", "🔋 Bateria"],
  },
};
let user = {},
...
function goTo(id) {
  ...
  nav.classList.toggle("show", !["splash", "login", "cadastro"].includes(id));
}
function setNav(id) {
  document
...
function fazerLogin() {
  ...
  goTo("home");
  setNav("home");
  showToast("✅ Bem-vindo de volta, João!");
}
function fazerCadastro() {
  ...
  user = { nome, sobrenome: sob, email, tel, cidade: cid, estado: est, cpf };
  updateUI();
  goTo("home");
  setNav("home");
...
function openAgenda(nome) {
  ...
  slotSel = null;
  document.querySelectorAll(".aslot").forEach((s) => s.classList.remove("sel"));
